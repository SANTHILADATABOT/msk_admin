...
if($_GET['id_number']=="")
{
 ...
 $select_customer_creation = $pdo_conn->prepare("SELECT * FROM matrimonial_information WHERE status='1' $user_type_query $dash_query ORDER BY matrimonial_id DESC");
 ...
 $select_customer_creation = $pdo_conn->prepare("SELECT * FROM matrimonial_information WHERE status='1' $query $user_type_query $dash_query ORDER BY matrimonial_id DESC");
  $select_customer_creation->execute();
  $survey_list = $select_customer_creation->fetchAll();
...
jQuery.ajax({
	...
	{ 
		//alert(msg);
		window.open('matrimonial_report/matrimonial_info_overall_print.php?id_number='+id_number+"&dash_country_id="+dash_country_id+"&dash_state_id="+dash_state_id+"&dash_district_id="+dash_district_id+"&dash_city_id="+dash_city_id+"&dash_area_id="+dash_area_id,'onmouseover','height=650,width=950,scrollbars=yes,resizable=no,left=250,top=250,toolbar=no,location=no,directories=no,status=no,menubar=no');	
	}
	});
