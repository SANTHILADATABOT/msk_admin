...
if($_GET['id_number']=="")
{
 ...
 $select_customer_creation = $pdo_conn->prepare("SELECT * FROM matrimonial_information WHERE status='1'  ORDER BY matrimonial_id DESC");
 ...
 $select_customer_creation = $pdo_conn->prepare("SELECT * FROM matrimonial_information WHERE status='1' $query ORDER BY matrimonial_id DESC");
  $select_customer_creation->execute();
  $survey_list = $select_customer_creation->fetchAll();
