...
if($_POST['action'] == 'state_list'){
	echo "SELECT * FROM state WHERE country_id=$_POST[country_id] and status='1' and delete_status='0' ORDER BY state_id ASC";
...
if($_POST['action'] == 'area_list'){
	...
	$city = $pdo_conn->prepare("SELECT * FROM area WHERE district_id=$_POST[district_id] and  state_id = $_POST[state_id] and country_id=$_POST[country_id]    and city_id=$_POST[city_id]  and status='1' and delete_status='0'");
	$city->execute();
	$city_list = $city->fetchall();
