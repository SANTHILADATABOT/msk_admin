...
$survey_sub_medical = $pdo_conn->prepare("SELECT DISTINCT a.country_id,a.district_id,a.state_id,a.city_id,a.area_id,a.survey_id,a.unique_no,a.family_no,b.disease_details,b.surgery_details,b.surgery_details_no,b.mon_exp_on_medicine,b.mon_exp_on_medicine_no,a.contact_no FROM  fact_family_disease b left join fact_finding_form a ON a.unique_no=b.unique_no WHERE  b.delete_status!='1' and a.user_id!='0' and a.delete_status!='1' $area_query ORDER BY a.survey_id DESC");
    ...
    $survey_sub_medical = $pdo_conn->prepare("SELECT DISTINCT a.country_id,a.district_id,a.state_id,a.city_id,a.area_id,a.survey_id,a.unique_no,a.family_no,b.disease_details,b.surgery_details,b.surgery_details_no,b.mon_exp_on_medicine,b.mon_exp_on_medicine_no,a.contact_no FROM  fact_family_disease b left join fact_finding_form a ON a.unique_no=b.unique_no WHERE  b.delete_status!='1' and a.user_id!='0' and a.delete_status!='1' AND b.disease_details!='' and b.mon_exp_on_medicine!='' $area_query ORDER BY a.survey_id DESC");
}						
 ...
 if(!empty($med_survey_list_sub))
 {
         foreach ($med_survey_list_sub as $record1)
         {
                ...
                $name='';
	    foreach($names as $na){
	        $sur_sub = $pdo_conn->prepare("SELECT * FROM fact_finding_subform WHERE id='$na' ORDER BY id DESC");
	        $sur_sub->execute();
            $sur_list_sub = $sur_sub->fetchall();
            
             foreach($sur_list_sub as $res){
                 $name .= $res['family_head_name'].', ';
             }
...
<td align="center" class="style2 right bottom">&nbsp;<?php if($record1['mon_exp_on_medicine_no']=='Select' || $record1['mon_exp_on_medicine_no']==''){echo '-';}else { echo rtrim($name,', ');} ?></td>
						<td align="center" class="style2 right bottom">&nbsp;<?php echo $record1[disease_details]; ?></td>
						<td align="center" class="style2 right bottom">&nbsp;<?php if($record1[mon_exp_on_medicine]==''){echo '-'; } else { echo ($record1[mon_exp_on_medicine]); } ?></td>
<?php
}
?>
