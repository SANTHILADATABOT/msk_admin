...
<section class="content-header report-pg" style="background-repeat: no-repeat;background-position: center;background-size: cover;background-image: url(images/top_bg.png);padding: 4px 30px;">
	  ...
	  <div class="col-md-6">
		...
		<div class="col-md-6">
		<div class="btn-add"><a href="index.php?file=surgical/create" >Add New</a></div>
		</div>
      </h1>	  
    </section>
...
//echo "SELECT * FROM  blood_group order by blood_id desc";
                  $surgical=$pdo_conn->prepare("SELECT * FROM  surgical order by surgical_id desc");
                  $surgical->execute();
                  $surgicallist=$surgical->fetchAll();
					?>
					<tbody>
						<?php foreach($surgicallist as $value)
						{ 
						?>
							<tr>
							  <td><?php echo $roll_id;?></td>
							  <td><?php echo $value['surgical_name'];?></td>
							 <td><?php echo $value['description']; ?></td>
							  <td><?php if ($value['status']=='1'){ echo "Active";} else { echo "Inactive"; }?></td>
							  <td> 
								<a href="index.php?file=surgical/update&surgical_id=<?php echo $value['surgical_id']?>" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a>  	
								<a href="#" onclick="del(<?php echo $value['surgical_id']?>)" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i></a>
							  </td>
							</tr>								
						<?php 
						$roll_id+=1;
