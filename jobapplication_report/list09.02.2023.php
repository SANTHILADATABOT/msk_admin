...
if($_GET['id_number']=="" && $_GET['qualification']=="")
{
 ...
 $select_customer_creation = $pdo_conn->prepare("SELECT * FROM job_application WHERE status='1' $user_type_query ORDER BY job_id DESC");
 ...
 $select_customer_creation = $pdo_conn->prepare("SELECT * FROM job_application WHERE status='1' $query $user_type_query ORDER BY job_id DESC");
  $select_customer_creation->execute();
  $survey_list = $select_customer_creation->fetchAll();
...
jQuery.ajax({
	...
	{ 
		//alert(msg);
		window.open('jobapplication_report/job_application_overall_print.php?id_number='+id_number+"&qualification="+qualification,'onmouseover','height=650,width=950,scrollbars=yes,resizable=no,left=250,top=250,toolbar=no,location=no,directories=no,status=no,menubar=no');	
	}
	});
