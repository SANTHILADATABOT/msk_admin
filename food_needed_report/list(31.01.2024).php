...
<table border="0" cellspacing="0" width="90%">
 ...
 //echo "SELECT * FROM fact_finding_form WHERE delete_status!='1' $query ORDER BY survey_id DESC";
						$survey = $pdo_conn->prepare("SELECT * FROM food_needed WHERE delete_status!='1' $query $user_type_query $dash_query ORDER BY food_needed_id DESC");
						$survey->execute();
...
jQuery.ajax({
	...
	{ 
		//alert(msg);
		window.open('food_needed_report/food_needed_overall_print.php?country_id='+country_id+'&state_id='+state_id+'&district_id='+district_id+'&city_id='+city_id+'&area_id='+area_id+"&dash_country_id="+dash_country_id+"&dash_state_id="+dash_state_id+"&dash_district_id="+dash_district_id+"&dash_city_id="+dash_city_id+"&dash_area_id="+dash_area_id,'onmouseover','height=650,width=950,scrollbars=yes,resizable=no,left=250,top=250,toolbar=no,location=no,directories=no,status=no,menubar=no');	
	}
	});
