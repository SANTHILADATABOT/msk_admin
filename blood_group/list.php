...
<section class="content-header report-pg" style="background-repeat: no-repeat;background-position: center;background-size: cover;background-image: url(images/top_bg.png);padding: 4px 30px;">
	  ...
	  <div class="col-md-6">
		...
		<div class="col-md-6">
		<div class="btn-add"><a href="index.php?file=blood_group/create" >Add New</a></div>
		</div>
      </h1>	  
    </section>
    <!-- Main content -->
...
//echo "SELECT * FROM  blood_group order by blood_id desc";
                  $bloodgroup=$pdo_conn->prepare("SELECT * FROM  blood_group order by blood_id desc");
                  $bloodgroup->execute();
                  $blood=$bloodgroup->fetchAll();
					?>
					<tbody>
						...
						?>
							<tr>
							  ...
							  <td><?php echo $value['blood_group_name'];?></td>
							 <td><?php echo $value['description']; ?></td>
							  <td><?php if ($value['status']=='1'){ echo "Active";} else { echo "Inactive"; }?></td>
							  <td> 
								<a href="index.php?file=blood_group/update&blood_id=<?php echo $value['blood_id']?>" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a>  	
								<a href="#" onclick="del(<?php echo $value['blood_id']?>)" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i></a>
							  </td>
							</tr>								
						<?php 
						$roll_id+=1;
